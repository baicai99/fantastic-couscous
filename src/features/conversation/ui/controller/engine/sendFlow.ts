...
import type { DraftSourceImageItem } from './useDraftSourceImages'
import type {
  Conversation,
  MessageAction,
  Run,
...
import { buildTextRequestMessages, resolveSendGenerationMode } from './sendFlowUtils'
import { createConversationTitleHelpers } from './sendFlowTitle'
import { createSendFlowRunActions } from './sendFlowRunActions'
import {
...
export function createSendFlowModule(deps: SendFlowDeps) {
  ...
  } = createConversationTitleHelpers({
    stateRef,
    ensureConversationLoaded,
    persistConversation,
  })
